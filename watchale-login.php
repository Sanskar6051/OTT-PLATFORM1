<?php   session_start();    ?>


<!DOCTYPE HTML>
<html>
<head>
<meta charset="utf-8">
<title> WATCHALE </title>
<!-- CSS -->
<link rel="stylesheet" href="watchale-main.css"/>
<link rel="stylesheet" href="css/lightslider.css"/>
<!-- Java Script -->
<script src="js/JQuery3.3.1.js"></script>
<script src="js/lightslider.js"></script>

  <!-- Fav Icon -->
  <link rel="Shortcut icon" href="Watchale-1.png"/>
  <!--using FontAwesome--------------->
<script src="https://kit.fontawesome.com/c8e4d183c2.js" crossorigin="anonymous"></script>
  </head>
  <body>
       

        <!--navigation-------------->
        <nav>
            <!--logo--------------->
            <a href="#" class="logo">
            <img src="Watchale-2.png"/>
            </a>
            <!--menu-------------->
            <ul class="menu">
                <li><a href="#" >Home </a></li>
                <li><a href="#">Movies</a></li>
                <li><a href="#">TV Shows</a></li>
                <li><a href="#">Hollywood</a></li>

            </ul>
            <style>
                .menum{
                    border: 1px solid black;
                }

            </style>
            <ul class="menu">
            
                <li class="menum"><a href="Myprofile.php"> <?php echo("Hello ".$_SESSION['name']); ?> </a></li>
                

            </ul>

            <!--search------------->
            <div class="search">
                <input type="text" placeholder="Find Your Favourtie Movies"/>
                <!--search-icon----------->
                <i class="fas fa-search"></i>
                
            </div>

            <i class="fas fa-sign-out-alt"><a href="logout.php">Logout</a></i>
            
        </nav>

        <section id="main"> 
            <!-- Showcase -->
            <!-- Heading -->
            <h1 class="showcase-heading">Showcase</h1>

            <ul id="autoWidth" class="cs-hidden">
                
                <!-- Box-1 -->
                <li class="item-a"> 
                    <!-- Showcase Box -->
                    <div class="showcase-box">
                    <a href="Video.html">
                        <img src="images/s-1.jpg" alt="Title-1">
                     </a>    
                    </div>
                </li>

                <!-- Box-2 -->
                <li class="item-b"> 
                    <!-- Showcase Box -->
                    <div class="showcase-box">
                    <a href="Video.html">
                        <img src="images/s-2.jpg" alt="Title-1">
                    </a>
                    </div>
                </li>

                <!-- Box-3 -->
                <li class="item-c"> 
                    <!-- Showcase Box -->
                    <div class="showcase-box">
                    <a href="Video.html">
                        <img src="images/s-3.jpg" alt="Title-1">
                    </a>
                    </div>
                </li>

                <!-- Box-4 -->
                <li class="item-d"> 
                    <!-- Showcase Box -->
                    <div class="showcase-box">
                        <img src="images/s-4.jpg" alt="Title-1">
                    </div>
                </li>

                <!-- Box-5 -->
                <li class="item-e"> 
                    <!-- Showcase Box -->
                    <div class="showcase-box">
                        <img src="images/s-5.png" alt="Title-1">
                    </div>
                </li>
                
              </ul>

        </section> 
        
        <!-- Latest Movies -->
        <section id="latest">

            <h2 class="latest-heading">Latest-Movies</h2>
            <!-- Slider in Latest-Movies -->

            <ul id="autoWidth2" class="cs-hidden">
                <!-- SLider-Box-1 -->
                <li class="item-a">
                    <div class="latest-box">
                        <!-- image -->
                        <div class="latest-b-image"> 
                            <img src="images/m-1.jpg" alt="">
                        </div>
                        <!-- text -->
                        <div class="latest-b-text">
                            <strong>Kin 2018</strong>
                            <p>Action Movies</p>
                        </div>

                    </div>
                </li>

                <!-- SLider-Box-2 -->
                <li class="item-b">
                    <div class="latest-box">
                        <!-- image -->
                        <div class="latest-b-image"> 
                            <img src="images/m-2.jpg" alt="">
                        </div>
                        <!-- text -->
                        <div class="latest-b-text">
                            <strong>Kin 2018</strong>
                            <p>Action Movies</p>
                        </div>

                    </div>
                </li>

                <!-- SLider-Box-3 -->
                <li class="item-c">
                    <div class="latest-box">
                        <!-- image -->
                        <div class="latest-b-image"> 
                            <img src="images/m-3.jpg" alt="">
                        </div>
                        <!-- text -->
                        <div class="latest-b-text">
                            <strong>Kin 2018</strong>
                            <p>Action Movies</p>
                        </div>

                    </div>
                </li>

                <!-- SLider-Box-4 -->
                <li class="item-d">
                    <div class="latest-box">
                        <!-- image -->
                        <div class="latest-b-image"> 
                            <img src="images/m-4.jpg" alt="">
                        </div>
                        <!-- text -->
                        <div class="latest-b-text">
                            <strong>Kin 2018</strong>
                            <p>Action Movies</p>
                        </div>

                    </div>
                </li>

                <!-- SLider-Box-5 -->
                <li class="item-e">
                    <div class="latest-box">
                        <!-- image -->
                        <div class="latest-b-image"> 
                            <img src="images/m-5.jpg" alt="">
                        </div>
                        <!-- text -->
                        <div class="latest-b-text">
                            <strong>Kin 2018</strong>
                            <p>Action Movies</p>
                        </div>

                    </div>
                </li>

                <!-- SLider-Box-6 -->
                <li class="item-f">
                    <div class="latest-box">
                        <!-- image -->
                        <div class="latest-b-image"> 
                            <img src="images/m-6.jpg" alt="">
                        </div>
                        <!-- text -->
                        <div class="latest-b-text">
                            <strong>Kin 2018</strong>
                            <p>Action Movies</p>
                        </div>

                    </div>
                </li>
                

                <!-- SLider-Box-7 -->
                <li class="item-g">
                    <div class="latest-box">
                        <!-- image -->
                        <div class="latest-b-image"> 
                            <img src="images/m-7.jpeg" alt="">
                        </div>
                        <!-- text -->
                        <div class="latest-b-text">
                            <strong>Kin 2018</strong>
                            <p>Action Movies</p>
                        </div>

                    </div>
                </li>

                <!-- SLider-Box-8 -->
                <li class="item-h">
                    <div class="latest-box">
                        <!-- image -->
                        <div class="latest-b-image"> 
                            <img src="images/s-1.jpg" alt="">
                        </div>
                        <!-- text -->
                        <div class="latest-b-text">
                            <strong>Kin 2018</strong>
                            <p>Action Movies</p>
                        </div>

                    </div>
                </li>

              </ul>

        </section>

        <!-- Movies
        <div class="movies-heading">
            <h2 >Movies</h2>
        </div>
        <section id="movies-list"> -->

            <!-- Box-1 -->
            <!-- <div class="movies-box>"> -->
                <!-- Images -->
                <!-- <div class="movies-img">
                    <img src="images/l-1.jpg">
                </div> -->
                <!-- text -->
                <!-- <a href="#">
                    Pushpa (2021) Full Movie [In Hindi] With English Subtitles
                </a>
            </div>

        </section> -->


        <!-- Slider Script -->
        <!--movies---------------------------->
<div class="movies-heading">
    <h2>Movies</h2>
</div>
<section id="movies-list">
    <!--box-1------------------------>
    <div class="movies-box">
        <!--img------------>
        <div class="movies-img">
            <div class="quality">HDRip</div>
            <img src="images/l-1.jpg">
        </div>
        <!--text--------->
        <a href="Video.html">
            Tin (2018) Full Movie[In English] With English Subtitles | HDRip 1080p HD
        </a>
    </div>
     <!--box-2------------------------>
     <div class="movies-box">
        <!--img------------>
        <div class="movies-img">
            <div class="quality">HDRip</div>
            <img src="images/l-2.jpg">
        </div>
        <!--text--------->
        <a href="Video.html">
            Tin (2018) Full Movie[In English] With English Subtitles | HDRip 1080p HD
        </a>
    </div>
     <!--box-3------------------------>
     <div class="movies-box">
        <!--img------------>
        <div class="movies-img">
            <div class="quality">HDRip</div>
            <img src="images/l-3.jpg">
        </div>
        <!--text--------->
        <a href="Video.html">
            Tin (2018) Full Movie[In English] With English Subtitles | HDRip 1080p HD
        </a>
    </div>
     <!--box-4------------------------>
     <div class="movies-box">
        <!--img------------>
        <div class="movies-img">
            <div class="quality">HDRip</div>
            <img src="images/l-4.jpg">
        </div>
        <!--text--------->
        <a href="Video.html">
            Tin (2018) Full Movie[In English] With English Subtitles | HDRip 1080p HD
        </a>
    </div>
     <!--box-5------------------------>
     <div class="movies-box">
        <!--img------------>
        <div class="movies-img">
            <div class="quality">HDRip</div>
            <img src="images/l-5.jpg">
        </div>
        <!--text--------->
        <a href="Video.html">
            Tin (2018) Full Movie[In English] With English Subtitles | HDRip 1080p HD
        </a>
    </div>
     <!--box-6------------------------>
     <div class="movies-box">
        <!--img------------>
        <div class="movies-img">
            <div class="quality">HDRip</div>
            <img src="images/l-6.jpg">
        </div>
        <!--text--------->
        <a href="Video.html">
            Tin (2018) Full Movie[In English] With English Subtitles | HDRip 1080p HD
        </a>
    </div>
     <!--box-7------------------------>
     <div class="movies-box">
        <!--img------------>
        <div class="movies-img">
            <div class="quality">HDRip</div>
            <img src="images/l-7.jpg">
        </div>
        <!--text--------->
        <a href="Video.html">
            Tin (2018) Full Movie[In English] With English Subtitles | HDRip 1080p HD
        </a>
    </div>
     <!--box-8------------------------>
     <div class="movies-box">
        <!--img------------>
        <div class="movies-img">
            
            <div class="quality">HDRip</div>
            <img src="images/l-8.jpg">
        </div>
        <!--text--------->
        <a href="Video.html">
            Tin (2018) Full Movie[In English] With English Subtitles | HDRip 1080p HD
        </a>
    </div>
</section>
<!--btns--------------->
<!-- <div class="btns">
    <a href="#">Previous</a>
    <a href="#">Next</a>
</div> -->
<!--footer------------------>
<footer>
    <p>Going To Internet, Ltd Consumer Website</p>
    <p>Copyright 2020 - GoingToInternet</p>
</footer>

        <script>
                        $(document).ready(function() {
                $('#autoWidth,#autoWidth2').lightSlider({
                    autoWidth:true,
                    loop:true,
                    onSliderLoad: function() {
                        $('#autoWidth,#autoWidth2').removeClass('cS-hidden');
                    } 
                });  
            });
        </script>

  </body>